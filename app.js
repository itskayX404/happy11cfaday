...
function renderAlbums(albums, photos) {
  ...
  albums.forEach((album, idx) => {
    ...
    // Album video: cover tetap pakai foto dari field cover
    const firstVideoSrc = isVideo ? ((album.photos || [])[0] || {}).src || '' : '';
...
/* Capture frame video — pakai autoplay+pause, satu-satunya cara works di mobile */
function captureVideoFrame(src, targetEl) {
  ...
  vid.autoplay = true;
  vid.loop = false;
  vid.style.cssText = 'width:100%;height:100%;object-fit:cover;position:absolute;inset:0;pointer-events:none;';
  targetEl.style.position = 'relative';
  targetEl.appendChild(vid);

  // Pause secepat mungkin setelah ada frame
    ...
    items.forEach((item, idx) => {
      ...
      if (isVid) {
        div.insertAdjacentHTML('beforeend', `
          <div class="av-vid-thumb" style="position:absolute;inset:0;overflow:hidden;"></div>
        ...
        const thumbSlot = div.querySelector('.av-vid-thumb');
        captureVideoFrame(item.src, thumbSlot);
      } else {
        div.innerHTML = `
...
function buildStrip() {
  ...
  lbPhotos.forEach((item, idx) => {
    ...
    if (item.type === 'video') {
      ...
      captureVideoFrame(item.src, thumb);
    } else {
      thumb.innerHTML = `<img src="${item.image}" alt="${item.caption || ''}"/>`;
    }
